function displayOperation_addMatrices() {
...
function executeOperation_addMatrices(showSteps) {
  ...
  if (matrix1_rows != matrix2_rows || matrix1_cols != matrix2_cols) {
    destination.innerHTML = "Invalid Operation. To add matrices they must be the same dimension.";
    return;
  }
  ...
  if (showSteps) {
      destination.innerHTML += "$$\r Add\\ the\\ elements\\ in\\ the\\ matching\\ positions: $$";
